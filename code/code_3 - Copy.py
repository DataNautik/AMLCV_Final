...
def _process_image(pil_img: Image.Image, low: float, high: float) -> Image.Image:
    ...
    # NEW: Apply CLAHE (Contrast Limited Adaptive Histogram Equalization)
    # This pulls details out of images that are too dark or too bright
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    gray = clahe.apply(gray)
    
    # 1. Lighter blur to remove noise but keep texture
    blur = cv2.GaussianBlur(gray, BLUR_KERNEL_SIZE, sigmaX=0)
    
    # 2. Canny Edge Detection
    edges = cv2.Canny(blur, threshold1=int(low), threshold2=int(high))
    
    # 3. Less aggressive morphology
    # We use a tiny 2x2 kernel just to connect broken lines
    k = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
    
    # REMOVED MORPH_OPEN (it was deleting fine lines)
    # REDUCED MORPH_CLOSE iterations to 1 so lines don't become massive blobs
...
def precompute_dataset(raw_dir: str, proc_dir: str):
    ...
    print("Precomputing OpenCV edges to avoid DataLoader bottlenecks...")
    for f_name in raw_files:
        try:
            ...
            # Old multiplier was 3.5. We reduce it to 2.5
            rough = _process_image(img, CANNY_LOW * 2.5, CANNY_HIGH * 2.5)
            rough.save(os.path.join(rough_dir, f_name))
            
            # Stage 2 – refined (target x_0)
            # Old multiplier was 2.0. We reduce it to 1.2
            refined = _process_image(img, CANNY_LOW * 1.2, CANNY_HIGH * 1.2)
...
def build_conditional_unet() -> UNet2DModel:
    return UNet2DModel(
        sample_size=IMAGE_SIZE, in_channels=2, out_channels=1,
        layers_per_block=2, block_out_channels=(32, 64, 128, 256),
